        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    # ✅ Pi2로 JSON 전송
                    try:
                        PI2_IP = "192.168.137.53"  # 실제 Pi2 IP로 수정
                        PI2_PORT = 5050
                        FILE_PATH = self.valence_arousal_path

                        with open(FILE_PATH, "r", encoding="utf-8") as f:
                            data = json.load(f)
                        serialized = json.dumps(data).encode("utf-8")

                        with socket.create_connection((PI2_IP, PI2_PORT), timeout=5) as sock:
                            sock.sendall(serialized)
                            print("✅ Pi2로 JSON 전송 완료")
